...
import { AttendanceApi, CustomersApi, TiffinTypesApi, ApiError } from "@/lib/api-client";
...
export default function AttendancePage() {
  ...
  async function handleSelect(customerId: string, meal: Meal, typeId: string | null) {
    ...
    try {
      if (typeId === null) {
        ...
        const updated = await AttendanceApi.update(existing.id, { tiffin_type_id: typeId });
  ...
  return (
    <div className="ledger-page min-h-full">
      ...
      {error && (
        <div className="mx-5 mb-3 text-sm text-brand-dark bg-brand/10 rounded-lg px-3 py-2">{error}</div>
      )}
        ...
        <div className="px-5 pb-6 flex flex-col gap-3">
          {customers.map((customer) => (
            <CustomerAttendanceRow
              ...
              customer={customer}
              types={types}
              lunchTypeId={records.get(key(customer.id, "LUNCH"))?.tiffin_type_id || null}
              dinnerTypeId={records.get(key(customer.id, "DINNER"))?.tiffin_type_id || null}
              onSelectLunch={(typeId) => handleSelect(customer.id, "LUNCH", typeId)}
              onSelectDinner={(typeId) => handleSelect(customer.id, "DINNER", typeId)}
              busy={busyKey === key(customer.id, "LUNCH") || busyKey === key(customer.id, "DINNER")}
            />
          ))}
        </div>
      )}
      ...
      {!loading && customers.length > 0 && (
        <div className="sticky bottom-0 mx-5 mb-4 bg-ink text-paper rounded-card px-4 py-3 flex items-center justify-between shadow-ledger">
          <div>
            <p className="text-[10px] uppercase tracking-wide text-paper/60">Day total</p>
            <p className="font-mono font-semibold">{totalTiffins} tiffins</p>
          </div>
          <p className="font-mono font-bold text-lg">{formatCurrency(totalAmount)}</p>
        </div>
      )}
    </div>
  );
